...
function parseGoal(value) {
  ...
  if (!text || text === '-' || text.toLowerCase() === 'p') return null;
...
function extractCountryAndCompetition(match) {
  ...
  if (container.length) {
    header = cleanHeaderText(container.find('.header').first().text());
  }

  if (!header) {
    header = cleanHeaderText(match.prevAll('.header').first().text());
  }
...
function extractCompareUrl(match, home, away) {
  const href = match
    .find('.actions .info a[href*="/compare/teams/"], a[href*="/compare/teams/"]')
    .first()
    .attr('href');
...
function extractScores(match) {
  const homeGoals = parseGoal(match.find('.hostteam .goals').first().text());
  const awayGoals = parseGoal(match.find('.guestteam .goals').first().text());
...
async function fetchFixturesByDate(date = formatTargetDate(0)) {
  ...
  $('.match').each((_, el) => {
    ...
    const score = extractScores(match);
